...
- Monthly Taxi Business Records ledger reading.
- Remittance-to-ledger matching using ledger date, remittance reference and Settlement amount.
- Preservation of the matched Google Sheets row number.
- Exact-match detection for validated CTL remittances.
- Automatic `Pending → Verified` status updates for exact matches only.
- Protection against repeat verification of rows already marked `Verified`.
- Idempotent repeat processing for previously verified remittances.

### Remittance Validation

- Validation that remittance invoice dates agree with remittance reference numbers.
- Invalid or inconsistent remittances are marked `REVIEW_REQUIRED` and excluded from automatic ledger verification.
- Protection against downloaded PDF filename collisions by prefixing attachment filenames with the Gmail message ID.

### Verification Audit Trail

- `VerificationMethod` type with:
  - `AUTOMATIC`
  - `MANUAL`
- `VerificationRecord` structure.
- Reading of the `Finance Agent Log` worksheet.
- Verification audit-log writing through `SheetsAgent`.
- Audit records containing:
  - Ledger Date
  - Method
  - Verified At
  - Source
- Automatic verification records include the source Gmail message ID.

### Manual Verification

- Protected manual ledger verification method.
- Explicit `verify-manual` Finance Agent CLI command.
- Manual verification permitted only when the target ledger row is currently `Pending`.
- Manual verification records the method as `MANUAL`.
- Manual verification preserves a supplied evidence/source description.
- Safety rejection for:
  - Missing command arguments.
  - Blank or non-Pending rows.
  - Rows already marked `Verified`.
...
- Gmail remittance processing now returns and processes multiple remittance records rather than relying on a single remittance.
- Account Booking candidates remain separate from the standard remittance reconciliation path.
- Automatic CTL remittance verification is restricted to internally `VALID` remittance records.
- Non-matching remittances leave the ledger unchanged.
- Ledger verification updates only the Status cell rather than rewriting unrelated ledger values.

### Documentation

- Updated `README.md` to reflect current reconciliation, verification and audit behaviour.
- Updated `PRINCIPLES.md` with explicit validation-versus-verification rules and automatic/manual verification principles.
- Updated `ROADMAP.md` to reflect the completed CTL reconciliation foundation, current verification milestone and next Account Booking reconciliation milestone.

---

## Validation

Live CTL reconciliation confirmed:

- 16/08 — exact CTL Settlement match → `Verified`.
- 17/08 — exact CTL Settlement match → `Verified`.
- 14/08 — non-matching CTL component → remained `Pending`.
- 15/08 — non-matching CTL component → remained `Pending`.
- Older inconsistent remittance → `REVIEW_REQUIRED` and excluded from automatic verification.

Repeat Finance Agent execution confirmed that already verified rows are not changed again.

Manual verification safety checks confirmed that non-Pending and already verified rows are rejected without ledger changes.

---

## Engineering

- Added remittance reference/date consistency test coverage.
- Added row-number support to remittance matching.
- Added Google Sheets ledger status update support.
- Added Finance Agent Log read/write support.
- Added protected manual verification support.
...
- Gmail processing still uses a limited recent-message search during development.
- Manual verification currently uses the Finance Agent command-line interface.
- Mixed CTL/operator payments remain pending until genuinely investigated.
